...
import cv2
# make sure to set and unset env variable $PYTHONPATH in the virtualenv using both files postactivate & postdeactivate
# located under /home/chrischen/.virtualenvs/your_venv_name/bin where your_venv_name is the name of your virtual env
from uvc_camera import UVC_Camera
...
def valid(datacfg0, datacfg1, datacfg2, datacfg3, cfgfile, conf_th):
    ...
    camera.start()
    cv2.namedWindow("6D pose estimation - multi-objects", cv2.WINDOW_AUTOSIZE)

    if not camera.video_capture.isOpened():
        # Cameras did not open, or no camera attached
        print("[Error] unable to open any cameras, check the number of devices in /dev/video? or cable connection")
        # clean up
        camera.stop()
...
def read_camera(uvc_camera, display_fps):
    """[summary: Read a frame from the camera, and draw the FPS on the image if desired]

    Args:
        uvc_camera ([type]): [description]
        display_fps ([type]): [description]

    Returns:
        [type]: [description: an image]
    """
    _, camera_image = uvc_camera.read()
    if display_fps:
        draw_label(camera_image, "Frames Displayed/sec : " +
                   str(uvc_camera.last_frames_displayed), (10, 20))
        draw_label(camera_image, "Frames Read/sec : " +
                   str(uvc_camera.last_frames_read), (10, 40))
    return camera_image



def draw_label(cv_image, label_text, label_position):
    """[summary: Simple draw label on an image; in our case, the video frame]

    Args:
        cv_image ([type]): [description]
        label_text ([type]): [description]
        label_position ([type]): [description]
    """
